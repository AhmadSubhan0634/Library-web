...
Status: Assignments 1–6 complete (Composer Autoloading, Router, PDO Database Layer, MVC/View separation, Authentication, Session-Protected Routes). Assignments 7–10 are not yet implemented.
...
How session-based authentication protects routes from unauthorized access
...
Session-based route protection — all book management routes (index, show, create, edit, update, delete) check for an active session before allowing access. Unauthorized users are redirected to the login page, preventing direct URL access to protected pages.

Assignment 1 — Composer Autoloading
Goal: Replace manual require_once statements with Composer's PSR-4 autoloading.

What was done
Ran composer init to generate composer.json

Configured PSR-4 autoloading, mapping App\ to app/

Created placeholder classes to confirm the autoloader resolves every namespace correctly
...
What was done
Built App\Core\Router with get()/post() to register routes, and resolve() to match the current request and dispatch it

Graceful error handling — 404 for unmatched routes, 500 for malformed/missing controller-method pairs

Built the supporting data layer: Book model, BookRepositoryInterface, JsonBookRepository (temporary), BookMapper, LibraryService
...
What was done
Built App\Core\Database — wraps a single, shared PDO connection, with a disconnect() method to release it

Designed a normalized schema: books, authors, category tables

Built App\Repositories\MySqlBookRepository, swapped in place of JsonBookRepository with no changes required to BookController or LibraryService

BookController's constructor wraps database/repository setup in a try/catch, storing a user-facing error message instead of letting an exception propagate
...
What was done
Built App\Core\View to centralize template rendering

Removed HTML from BookController and HomeController — each action gathers data and hands it to a View

Created view files under app/Views/books/ and app/Views/home/, plus a shared errors/error.php
...
What was done
Created a users table (id, username, password, created_at, updated_at)

Built App\Models\User — a plain data object mirroring the users table, with getters/setters only

Built App\Repositories\UserRepositoryInterface, defining findByUsername() and create()

Built App\Repositories\MySqlUserRepository, implementing the interface with prepared statements

Built App\Controllers\AuthController:
...
Registered routes in public/index.php:
...
Added session_start() to public/index.php so session data is available to every controller
...
Goal: Restrict access to book management routes so only logged-in users can view, add, edit, or delete books. Unauthenticated users should be redirected to the login page.

What was done
Added session checks to all protected routes in BookController:

index() — redirects to /login if no active session

show() — redirects to /login if no active session

create() — redirects to /login if no active session

store() — redirects to /login if no active session

edit() — redirects to /login if no active session

update() — redirects to /login if no active session

destroy() — redirects to /login if no active session

Protected all book-related CRUD operations:

All seven methods now check if (!isset($_SESSION['user_id'])) before executing any logic

Unauthorized users are redirected with header('Location: /login') and exit

Ensured consistent protection:

Every route that interacts with books requires authentication

The home page (/) remains publicly accessible

Login and registration pages remain publicly accessible
...
This pattern is repeated across all book management methods, ensuring consistent security throughout the application.

Validation
Without logging in:

Accessing /books → Redirects to /login

Accessing /books/create → Redirects to /login

Accessing /books/edit?isbn=1 → Redirects to /login

Accessing any book route → Redirects to /login

After logging in:

Accessing /books → Shows the book list

Accessing /books/create → Shows the add book form

Accessing /books/edit?isbn=1 → Shows the edit form

All CRUD operations work as expected

After logging out:

All protected routes redirect to /login again

Security Benefits
Prevents direct URL access — users cannot bypass authentication by typing protected URLs directly

Protects all CRUD operations — creates, reads, updates, and deletes all require authentication

Consistent enforcement — the same session check pattern is used across all protected methods

Clear user experience — users are always redirected to the login page with a clear path to access

Known limitations (intentional or pending cleanup)
~~Routes are not yet gated by login state — any visitor can still reach /books/create, /books/edit, etc. regardless of session status. Restricting these to logged-in users is Assignment 6.~~ COMPLETED ✅

AuthController::showError() still uses raw echo HTML, unlike BookController::showError(), which was updated in Assignment 4 to render errors/error.php via View::render(). This is a leftover inconsistency to fix as a follow-up.

Database credentials are currently passed directly in each controller's constructor rather than loaded from environment variables — to be addressed as a follow-up cleanup.
